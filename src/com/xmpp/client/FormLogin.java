...
//			final String PWD = this.pwdText.getText().toString();
			final String USERID = "liuwei@smalltreemedia.com";
			final String PWD = "sxylw5528";
			new Thread(new Runnable() {				
				...
				public void run() {
					...
					try {
						...
						XMPPConnection conectionConnection = XmppTool.getConnection();
					    Roster roster = conectionConnection.getRoster();
					    
				        Collection<RosterEntry> entries = roster.getEntries();  
				        for (RosterEntry entry : entries) {
//				        	ItemStatus status = entry.getStatus();
				            System.out.println(String.format("Buddy:" + entry.getName() +"- user:"+ entry.getUser()+" - Status:" + entry.getStatus()));
				        }
